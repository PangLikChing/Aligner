...
public class FlappyBirdSpawningHoldLength : MonoBehaviour
{
    [SerializeField] Transform canvas;
    FlappyBirdLevelManager flappyBirdLevelManager;
    FlappyBirdSpawningNormal flappyBirdSpawningNormal;

    GameObject scoreText, gameoverPanel;

    float length, minLength, maxLength, gapSize, offset;

    public float skilltime, skillColliderLength;
    ...
    void Awake()
    {
        ...
        flappyBirdSpawningNormal = gameObject.GetComponent<FlappyBirdSpawningNormal>();
    }
    ...
    void Update()
    {
        if (flappyBirdLevelManager.isAlive)
        {
            ...
            if (skilltime <= 0)
            {
                flappyBirdSpawningNormal.enabled = true;
                this.enabled = false;
            }
        }
    }
